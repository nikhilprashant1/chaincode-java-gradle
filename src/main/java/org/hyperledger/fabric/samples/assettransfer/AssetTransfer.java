...
public final class AssetTransfer implements ContractInterface {
    ...
    public String TransferAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        // Extract peer identity from the TLS certificate
        String certSubject = ctx.getClientIdentity().getX509Certificate().getSubjectX500Principal().getName();
        String peerIdentity = extractPeerIdentity(certSubject);

        // Only allow transfers from org1-peer2
        if (!"org1-peer2".equals(peerIdentity)) {
            throw new ChaincodeException("Unauthorized: Only org1-peer2 can transfer assets", "UNAUTHORIZED");
        }
    ...
    // Helper method to extract peer identity
